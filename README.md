...
> **Yang perlu Anda lakukan:** jalankan ulang `pip install -r requirements.txt`
> sebelum `streamlit run app.py` — ada 2 dependency baru (`Pillow` untuk logo,
> `streamlit-option-menu` untuk navbar) yang belum ada di environment lama Anda.
> Kalau `streamlit-option-menu` gagal ter-install, aplikasi tetap jalan (fallback
> otomatis ke radio button biasa), tapi tampilan navbar-nya tidak akan se-elegan
> yang dituju di poin 1.
...
| Sheet | Isi |
|---|---|
| `Employees` | Data mentah pegawai: NIP, Nama, Unit, Satker, Pangkat (DD/AD/M/AM), Sublevel, Tanggal_Grade, NK_1..NK_5, Pendidikan, Sertifikasi, Exposure, Potensi, K3, Status, PTB_S2, Promotion_Award, Satker_Recommendation, Remaining_Service |
...
Catatan: kolom `Readiness` **tidak lagi disimpan sebagai data mentah** — sekarang
dihitung otomatis oleh `scoring.py` dari MDG tiap pegawai (lihat bagian
"Perubahan pada update ini" di bawah).
...
1. **Desain & navbar** — navbar sidebar diganti dari radio button ke
   `streamlit-option-menu` (pill-style, dengan ikon). Palet warna diganti dari
   merah default Streamlit ke navy/teal (lihat token warna di bagian atas
...
- **Passing Grade** Gate KPP dihitung sebagai rata-rata QScore/Quantitative/
  ...
  per pangkat.
- **Readiness** memakai asumsi bucket waktu (Ready Now / 1 tahun / 2 tahun /
  >2 tahun / Belum Siap) berdasarkan jarak MDG ke threshold — kalau BI punya
  definisi readiness yang lebih spesifik (mis. melibatkan penilaian atasan),
  logikanya tinggal disesuaikan di fungsi `readiness_row()` dalam `scoring.py`.
