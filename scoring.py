...
def compute_all(employees: pd.DataFrame, refs: dict) -> pd.DataFrame:
    ...
    # ---------- Readiness (dihitung dari MDG, bukan kolom acak) ----------
    def readiness_row(row):
        if row["Masuk_Proses_KPP"]:
            return "Ready Now"
        if row["Lolos_KPP"] and not row["Is_Senior"] and not row["Chk_MDG_Terpenuhi"]:
            sisa = mdg_threshold - row["MDG_Tahun"]
            if sisa <= 1:
                return "Ready 1 Tahun Lagi"
            if sisa <= 2:
                return "Ready 2 Tahun Lagi"
            return "Ready >2 Tahun Lagi"
        return "Belum Siap"
...
READINESS_ORDER = [
    "Ready Now", "Ready 1 Tahun Lagi", "Ready 2 Tahun Lagi",
    "Ready >2 Tahun Lagi", "Belum Siap",
]

READINESS_COLOR = {
    "Ready Now": "#3E7C74",
    "Ready 1 Tahun Lagi": "#7B9E3F",
    "Ready 2 Tahun Lagi": "#C08A2E",
    "Ready >2 Tahun Lagi": "#8C97A3",
    "Belum Siap": "#B0574A",
}
